...
def main():
    ...
    จากรูปภาพใบสั่งซื้อ (Purchase Order) ที่แนบมานี้ โปรดดึงข้อมูลต่อไปนี้ออกมาและจัดให้อยู่ในรูปแบบ JSON ที่ถูกต้องเท่านั้น:

    1.  `company_info`: ข้อมูลบริษัทที่อยู่บนหัวกระดาษของหน้าที่ 2
        -   `name`: ชื่อบริษัท
        -   `address`: ที่อยู่ทั้งหมด
        -   `tax_id`: เลขประจำตัวผู้เสียภาษี
    2.  `po_number`: หมายเลข PO ที่ระบุในเอกสาร
    3.  `items`: รายการสินค้าทั้งหมดจากทุกหน้า โดยแต่ละรายการให้มีข้อมูลดังนี้
        -   `item_number`: หมายเลข Item
        -   `description`: รายละเอียดสินค้า
        -   `quantity`: จำนวน
        -   `unit`: หน่วยนับ (เช่น EA)
        -   `unit_price`: ราคาต่อหน่วย
        -   `total`: ราคารวมของรายการนั้น

    ข้อกำหนดเพิ่มเติม:
    - หากข้อมูลใดหาไม่เจอ ให้ใส่ค่าเป็น `null`
    - สำหรับข้อมูลตัวเลข (quantity, unit_price, total) ให้แปลงเป็น Number Type (ไม่ต้องมี comma)
